...
```{r}
road <- st_read("../RoadNetworks/Fraybentos/Fraybentos_Network_Cleaned.shp")
...
```{r}
EP <- st_read("../PublicSpaces/Fraybentos/Points_Fraybentos.shp") %>% 
  st_transform(st_crs(road)$epsg)


Polygon <- st_read("../DatosCenso/Fray_Bentos.shp") %>% 
...
```{r}
MinDistances_EP_2 <- MinDistances_EP %>% st_set_geometry(NULL)
```


```{r}
Ref <- seq(0,5000,200) #Double check the units
...
for (i in seq_along(Ref)){
  ...
  for (j in 1:dim(MinDistances_Population)[1]){
    ...
    ##
    DistTemp %<>% left_join(MinDistances_EP_2, by = c("NodeDestination" = "Node"))
    DistTemp_Final <- DistTemp %>% group_by(EspacioID) %>% summarise(Total = n())
    ##
    
    TotalOpportunities <- dim(DistTemp_Final)[1]  
...
write_delim(Population_OA, "Results/Fraybentos/MatrixOA_Fraybentos")
st_write(MinDistances_Population, "Results/Fraybentos/Pouplation_Fraybentos.shp")
